...
object ChatModel {
  ...
  // Function removed from here, will be added inside ChatController
}
...
data class CIFile(
  ...
  val id: Long get() = fileId // Added for consistency with ChatItem.id for pagination
...
suspend fun exportChatHistory(
    ...
    // Call the helper function to attempt downloads and update paths
    val localCurrentUser = ChatModel.currentUser.value
    if (localCurrentUser != null && mediaToExportList.any { it.originalPath != null && it.originalPath.startsWith("needs_download/") }) {
...
// private suspend fun attemptMediaDownloadsAndUpdatePaths(...) { ... } // This is now defined below, or was defined above and moved.
